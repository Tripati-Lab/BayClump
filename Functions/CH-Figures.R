##https://raw.githubusercontent.com/astrobayes/BMAD/master/auxiliar_functions/CH-Figures.R
##BMAD: CH-Figures
...
MyBUGSHist <- function(xx, vars, col = c('grey',"#1f78b4"), priors){
  ...
  fullPosterior=cbind.data.frame(AllID, AllParams, group='Posterior')
  complete_pripo<-rbind.data.frame(fullPosterior,priors)
  
  
  Z<-ggplot(complete_pripo, aes(x = AllParams, fill=group)) +
    geom_histogram(alpha = .9, position = "identity") +
    facet_wrap( ~AllID , scales = "free")+
    xlab("")
  return(Z)
}
