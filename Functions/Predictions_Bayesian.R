...
predictTcBayes <- function(calibrationData, 
  ...
  if(bootDataset){
    ...
    res <- single_rep()
    post <- attr(res, "PosteriorOne")
    res$Tc <- sqrt(10^6/res$Tc)-273.15
    res$lwr <- sqrt(10^6/res$lwr)-273.15
    res$upr <- sqrt(10^6/res$upr)-273.15
    res$se <- (res$lwr - res$upr) / 3.92
    res$sd <- res$se * sqrt(res$n)
    to_ret <- res[,c(1:4,6,9)]
    attr(to_ret, "PosteriorOne") <- post
    to_ret
